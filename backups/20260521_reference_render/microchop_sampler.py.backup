#!/usr/bin/env python3
"""Render a generated melody MIDI phrase with tuned one-shot microchops."""
...
DEFAULT_SAMPLE = (
    "original_sample/"
    "Domenico_Scaratty_Sonata_Piano_Re_Minor_Domenico_Scarlatti_Sonata_For_Piano_D_Moll_"
    "3dHxqobZ90_Dmin_112bpm.wav"
)
...
def make_chops(args: argparse.Namespace, run_dir: Path) -> list[dict[str, Any]]:
...
def render_events(
    ...
    if not chop_index:
        raise RuntimeError("No pitched chops were detected; cannot render pitched one-shot sampler.")
    ...
    for event in midi_events:
        ...
        insert = int(round(event.start_sec * sr))
...
def write_reports(
    ...
    summary.extend(
        [
            "",
            "Future playback modes to consider: `gated`, `stretch`, `loop`, and `slice-sequence`.",
            "",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--playback-mode", default="one-shot", choices=["one-shot"])
...
def main() -> None:
    ...
    write_reports(run_dir, args, chops, midi_events, midi_info, event_map, render_stats)
